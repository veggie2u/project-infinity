...
- [x] **0.5 GitHub Actions CI** — `.github/workflows/ci.yml` running typecheck/lint/test/build via Turborepo on every PR and push to `main`, with pnpm + Turborepo caching. Confirmed working on a real PR (#4): all four steps passed, and a follow-up push confirmed both caches actually get reused (1m6s cold run → 35s cached run, `FULL TURBO` on build/lint/typecheck/test). `.github/workflows/eas-update-preview.yml` (EAS Update PR preview, via `expo/expo-github-action` + `EXPO_TOKEN`) has since been added and confirmed posting a real QR-code preview comment on PR #5. EAS Build-on-merge and the RLS-testing job remain deferred — see `TECH-STACK.md` §9/§5a for the full design once their prerequisites (a paid Apple Developer account, and real `supabase/migrations/`, respectively) exist.
