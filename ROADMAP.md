...
- [x] **0.5 GitHub Actions CI** — `.github/workflows/ci.yml` running typecheck/lint/test/build via Turborepo on every PR and push to `main`, with pnpm + Turborepo caching. Confirmed working on a real PR (#4): all four steps passed, and a follow-up push confirmed both caches actually get reused (1m6s cold run → 35s cached run, `FULL TURBO` on build/lint/typecheck/test). `.github/workflows/eas-update-preview.yml` (EAS Update PR preview, via `expo/expo-github-action` + `EXPO_TOKEN`) has since been added and confirmed posting a real QR-code preview comment on PR #5, with `eas update:configure` also run to properly set `runtimeVersion`/`updates.url`/channels (previously missing entirely). On-device QR verification in Expo Go is currently blocked by an external factor, not a repo issue — Expo Go's app-store build only supports up to SDK 54 as of this writing, while this project is on SDK 57 (see `SETUP-NOTES.md`); revisit once Expo Go's app-store release catches up. EAS Build-on-merge and the RLS-testing job remain deferred — see `TECH-STACK.md` §9/§5a for the full design once their prerequisites (a paid Apple Developer account, and real `supabase/migrations/`, respectively) exist.
